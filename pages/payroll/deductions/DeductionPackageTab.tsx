...
import Modal from '../../../components/Modal'; // Fixed path to root components
import {
    Plus,
    Trash2,
    Play,
...
const MOCK_DEDUCTION_PACKAGES: PayTemplate[] = [
    ...
    {
        id: 'ded-pkg-2',
        name: 'Executive Deduction Package',
        targetType: 'Rank',
        targetId: 'rank-3',
        components: ['comp-d1', 'comp-d2', 'comp-d3', 'comp-d5'],
    }
];
...
const MOCK_DEDUCTION_COMPONENTS: PayComponent[] = [
    ...
    },
    {
        ...
        frequency: 'Monthly',
        distribution: [
            { period: '2nd Pay Period', amount: 900 }
        ]
    },
    {
        id: 'comp-d3',
        ...
        frequency: 'Monthly',
        distribution: [
            { period: '1st Pay Period', amount: 200 }
        ]
    },
    {
        id: 'comp-d4',
    ...
    },
    {
        ...
        frequency: 'Weekly',
        distribution: [
            { period: 'Week 1', amount: 62.5 },
            { period: 'Week 2', amount: 62.5 },
            { period: 'Week 3', amount: 62.5 },
            { period: 'Week 4', amount: 62.5 }
        ]
    }
];

const DeductionPackageTab: React.FC = () => {
    // Internal State for Mock Data
    const [templates, setTemplates] = useState<PayTemplate[]>(MOCK_DEDUCTION_PACKAGES);
            ...
            <div className="w-full md:w-80 border-b md:border-b-0 md:border-r border-slate-100 flex flex-col bg-slate-50/50 shrink-0 h-[300px] md:h-auto">
                <div className="p-5 border-b border-slate-100 bg-white">
                    <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <FileSpreadsheet className="text-amber-500" size={20} /> Deduction Packages
                    </h2>
                    <p className="text-xs text-slate-500 mb-3 font-medium">Define mandated & custom deductions for groups.</p>
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search deduction packages..."
                            ...
                            <div className="flex-1 overflow-y-auto p-8 bg-slate-50/30">
                                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                                    <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                        <h3 className="text-sm font-bold text-slate-700">Covered Employees</h3>
                                    ...
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-white text-slate-400 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100">
                                            <tr>
                                                ...
                                                <th className="px-6 py-4 text-right">Deduction Variance</th>
                                                    ...
                                                    <td className="px-6 py-4 text-right font-mono text-slate-600 font-bold">
                                                        {inst.isCustomized ? (
                                                            <div className="flex flex-col items-end">
                                                                <span className="text-rose-600 bg-rose-50 px-2 py-1 rounded border border-rose-100">+ ₱{inst.netDiff.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                                <span className="text-[10px] text-slate-400 mt-1 font-bold uppercase tracking-tighter">More Deducted</span>
                                                            </div>
                                                        ) : (
                ...
                <div className="p-8">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="pt-4 flex gap-3">
                            <button onClick={handleSaveTemplate} className="flex-1 py-3 bg-slate-900 text-white rounded-xl font-bold shadow-lg shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95">{editingId ? 'Save Changes' : 'Create Package'}</button>
                            ...
                            <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-white z-10 shrink-0">
                                <div>
                                    <div className="flex items-center gap-3 mb-2">
                                        <h3 className="text-xl font-bold text-slate-900">Package Impact Analysis</h3>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-6">
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Mandated/Fixed Deduction</div>
                                    ...
                                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest mb-1 relative z-10">Current Actuals</div>
                                    ...
                                    <div className={`p-4 rounded-xl border shadow-sm ${comparisonData.summary.diff > 0 ? 'bg-emerald-50 border-emerald-100' : comparisonData.summary.diff < 0 ? 'bg-rose-50 border-rose-100' : 'bg-slate-50 border-slate-200'}`}>
                                        <div className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${comparisonData.summary.diff > 0 ? 'text-emerald-600' : comparisonData.summary.diff < 0 ? 'text-rose-600' : 'text-slate-400'}`}>Take Home Impact</div>
...
export default DeductionPackageTab;
