                ...
                {filteredSchedules.map(emp => (
                    <div key={emp.id} className="flex border-b border-slate-100 p-3 items-center hover:bg-slate-50 cursor-pointer">
                        <div className="w-56 shrink-0 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-600">{emp.avatar}</div>
                            <div>
            ...
            <div className="min-w-[800px]">
                {filteredSchedules.map(emp => (
                    <div key={emp.id} className="flex border-b border-slate-100 p-3 items-center hover:bg-slate-50 cursor-pointer">
                        ...
                        {['M', 'T', 'W', 'TH', 'F', 'S', 'SU'].map((d, index) => {
                            const workdaysArray = emp.workdays.split(', ');
                            const hasShift = workdaysArray.includes(d);
                            return (
                                <div key={index} className="flex-1 p-1">
                                    {hasShift ? (
                                        <div className="h-10 bg-indigo-50 border border-indigo-100 text-indigo-700 rounded-lg flex flex-col items-center justify-center text-[9px] font-bold text-center shadow-sm">
    ...
    const renderMonthView = () => (
        <div className="p-5">
            <div className="grid grid-cols-7 gap-2 mb-2">
                {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (
                    <div key={d} className="bg-slate-50 p-2 text-center text-[10px] font-bold uppercase tracking-widest text-slate-500 rounded-lg">{d}</div>
                ))}
            </div>
            <div className="grid grid-cols-7 gap-2">
                {Array.from({ length: 31 }).map((_, i) => (
                    <div key={i} className="border border-slate-100 min-h-[100px] p-2 rounded-xl hover:bg-slate-50 hover:border-indigo-100 transition-colors cursor-pointer group">
                        <div className="text-xs font-bold text-slate-400 group-hover:text-indigo-600">{i + 1}</div>
                        <div className="mt-2 space-y-1">
                            {i % 3 === 0 && (
                                <div className="text-[9px] font-bold bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md truncate border border-indigo-100">
                                    3 Shifts Scheduled
                                </div>
                            )}
                            {i % 5 === 0 && (
                                <div className="text-[9px] font-bold bg-emerald-50 text-emerald-700 px-2 py-1 rounded-md truncate border border-emerald-100">
                                    2 On Leave
                                </div>
                            )}
                            {i % 7 === 0 && (
                                <div className="text-[9px] font-bold bg-amber-50 text-amber-700 px-2 py-1 rounded-md truncate border border-amber-100">
                                    1 Request Pending
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="p-5 border-b border-slate-50 flex flex-col lg:flex-row gap-4 items-center justify-between bg-white">
                        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                            ...
                            <div className="relative border border-slate-200 rounded-xl hover:border-indigo-200 transition-all bg-white group focus-within:border-indigo-500">
                                <select
                                    value={scheduleView}
                                ...
                                <ChevronLeft className="absolute right-3 top-1/2 -translate-y-1/2 -rotate-90 text-slate-400 pointer-events-none group-hover:text-indigo-600" size={12} />
                            </div>
                        </div>
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {filteredSchedules.map((item) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide shadow-sm border ${item.status === 'Active'
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredLeaves.map((item) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide shadow-sm border ${item.approvalStatus === 'Approved'
                                                    ? 'bg-emerald-50 text-emerald-600 border-emerald-100'
