...
const MOCK_ADJUSTMENTS: AdjustmentHistory[] = [
    { id: 'adj-1', date: 'Aug 1, 2025', employees: ['James Cordon', 'Louis Panganiban'], leaveType: 'Vacation Leave', amount: 1, type: 'Credit', reason: 'Performance Reward', admin: 'Sarah Wilson', effectiveDate: 'Aug 1, 2025', isPaid: true },
    { id: 'adj-2', date: 'Jul 15, 2025', employees: ['Juan Dela Cruz'], leaveType: 'Sick Leave', amount: 0.5, type: 'Debit', reason: 'Adjustment Error', admin: 'Sarah Wilson', effectiveDate: 'Jul 15, 2025', isPaid: true },
];
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="col-span-full">
                                            ...
                                            <div className="flex bg-slate-100 p-1 rounded-xl h-[46px]">
                                                <button
                                                    onClick={() => setAdjustmentForm({ ...adjustmentForm, isPaid: true })}
                                                    className={`flex-1 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${adjustmentForm.isPaid ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                                >
                                                    With Pay
                                                </button>
                                                <button
                                                    onClick={() => setAdjustmentForm({ ...adjustmentForm, isPaid: false })}
                                                    className={`flex-1 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${!adjustmentForm.isPaid ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                                >
                                                    Without Pay
                                                </button>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {MOCK_ADJUSTMENTS.map(adj => (
                                                <tr key={adj.id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 align-top">
                                                        <span className={`text-[9px] font-black uppercase tracking-tighter px-2 py-0.5 rounded-full border ${adj.isPaid ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                                            {adj.isPaid ? 'With Pay' : 'Without Pay'}
                                                        </span>
                                                    </td>
