...
const PayTemplateCard = ({ data, type, onAmountChange, onAddComponent }: { data: any, type: 'current' | 'proposed', onAmountChange?: (index: number, newAmount: string, field?: 'amount' | 'name') => void, onAddComponent?: () => void }) => {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden ${borderColor} ${ringColor} ring-4 bg-white transition-all hover:shadow-lg h-full flex flex-col`}>
      <div className={`px-5 py-4 border-b ${headerColor} flex justify-between items-center relative`}>
        <span className="text-xs font-bold uppercase tracking-widest">{isProposed ? 'Proposal' : 'Current'}</span>
        {isProposed && <span className="text-[10px] font-bold bg-white/60 px-2 py-0.5 rounded text-emerald-800 backdrop-blur-sm shadow-sm relative z-10">+8.5% Increase</span>}
      </div>
      <div className="flex-1 overflow-x-auto relative z-20 bg-white">
        <table className="w-full text-sm mb-0">
          <tbody className="divide-y divide-slate-50">
            <tr>
              ...
              <td className={`px-5 py-3 text-right font-bold tabular-nums ${isProposed ? 'text-emerald-600' : 'text-slate-900'}`}>{isProposed && onAmountChange ? <input className="w-24 text-right bg-emerald-50/50 border border-emerald-200 focus:border-emerald-400 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 rounded px-2 py-1 outline-none font-mono" value={data.basic} onChange={e => onAmountChange(-1, e.target.value)} /> : `₱${data.basic}`}</td>
            </tr>
            <tr>
              <td colSpan={2} className="px-5 py-2 bg-slate-50/50 relative z-10 w-full overflow-visible">
                <div className="flex flex-col gap-2">
                  {data.components?.map((comp: any, i: number) => (
                    <div key={i} className="flex justify-between items-center text-xs">
                      <div className="flex items-center gap-1.5 flex-1 w-0 truncate">
                        {isProposed && onAmountChange && comp.isNew ? (
                          <input className="w-full text-left bg-emerald-50/50 border border-emerald-200 focus:border-emerald-400 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 rounded px-2 py-1 outline-none text-slate-600" value={comp.name} placeholder="Component Name" onChange={e => onAmountChange(i, e.target.value, 'name')} />
                        ) : (
                          <span className="text-slate-600 truncate">{comp.name}</span>
                        )}
                        {comp.needsPAF && !comp.isNew && (
                          <span className="text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-1.5 rounded flex items-center gap-0.5 shrink-0" title="Requires Personnel Action Form to change">
                            <AlertCircle size={8} /> PAF Req.
                          </span>
                        )}
                      </div>
                      <div className="shrink-0 pl-2">
                        {isProposed && onAmountChange ? (
                          <input className="w-24 text-right font-mono bg-emerald-50/50 border border-emerald-200 focus:border-emerald-400 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 rounded px-2 py-1 outline-none text-slate-600" value={comp.amount} placeholder="0.00" onChange={e => onAmountChange(i, e.target.value, 'amount')} />
                        ) : (
                  ...
                  {isProposed && onAddComponent && (
                    <button onClick={onAddComponent} className="w-full mt-2 py-2 border border-dashed border-emerald-200 text-emerald-600 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-emerald-50 transition-colors flex justify-center items-center gap-2">
            ...
            <tr className="bg-slate-50/50 border-t border-slate-100 relative z-20">
...
const JobHistoryTab: React.FC = () => {
  ...
  const [pafForm, setPafForm] = useState({
    ...
    proposedPayData: {
      ...
      components: [
        { name: 'Rice Subsidy', amount: '2,000.00', needsPAF: false, isNew: false as boolean | undefined },
                ...
                onClick={() => {
                  ...
                  setPafForm({
                    ...
                    proposedPayData: {
                      basic: '27,000.00', daily: '1,072.85', hourly: '134.11', net: '25,050.00',
                      components: [
                        { name: 'Rice Subsidy', amount: '2,000.00', needsPAF: false, isNew: false },
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-8 items-stretch relative">
                ...
                <div className="flex-1 space-y-3 z-20">
                  ...
                  <PayTemplateCard
                    data={pafForm.proposedPayData}
                    type="proposed"
                    onAmountChange={(idx, val, field) => {
                      if (idx === -1) {
                        setPafForm({
                          ...pafForm,
                          proposedPayData: { ...pafForm.proposedPayData, basic: val }
                        });
                      } else {
                        const newComps = [...pafForm.proposedPayData.components];
                        if (field === 'name') newComps[idx].name = val;
                        if (field === 'amount') newComps[idx].amount = val;
                        setPafForm({
                          ...pafForm,
                          proposedPayData: { ...pafForm.proposedPayData, components: newComps }
                        });
                      }
                    }}
                    onAddComponent={() => {
                      setPafForm({
                        ...pafForm,
                        proposedPayData: {
                          ...pafForm.proposedPayData,
                          components: [...pafForm.proposedPayData.components, { name: '', amount: '0.00', needsPAF: false, isNew: true }]
                        }
                      });
                    }}
                  />
